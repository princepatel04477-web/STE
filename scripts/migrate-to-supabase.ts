...
async function runMigration() {
  ...
  REGISTERED_EXHIBITORS_LIST.forEach((reg) => {
    ...
    exhibitorsToInsert.push({
      ...
      exhibitor_name: localEx?.exhibitor_name || null,
      profile_pic_url: localEx?.profile_pic_url || null,
      company_description: localEx?.company_description || null,
  ...
  (localStore.exhibitors || []).forEach((e: any) => {
    if (!exhibitorsToInsert.some((x) => x.mobile === e.mobile)) {
      exhibitorsToInsert.push({
        ...
        profile_pic_url: e.profile_pic_url || null,
